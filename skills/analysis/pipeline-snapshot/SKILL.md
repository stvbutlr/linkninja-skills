...
|-------|-----|------------|
| Pipeline has been classified | `get_stats()` — at least one stage has > 0 | "Your pipeline is empty. Classify some conversations first — try **full-morning-triage** or `start_batch_classify()`." |
| Stages defined | `list_stages()` returns the user's stage list | Use the 7-stage default (opening, chatting, qualified, discovery, closing, won, lost). |
...
claude schedule create \
  ...
  --allowed-tools "get_context,get_stats,Read,Write"
```
...
<dict>
  ...
  <array>
    ...
    <string>bash</string>
    <string>-c</string>
    <string>claude -p "$(cat ~/.claude/routines/pipeline-daily-brief.prompt.md)" --allowedTools "get_context,get_stats,Read,Write"</string>
  </array>
...
Caps rationale: the skill calls `get_context` (1) + `get_stats` (1) + `Read` previous (1) + `Write` new (1) = 4 tool calls. Cap at 6 leaves slack for one optional `search_conversations` if the heads-up line needs a count. Zero drafts because this skill never drafts.
...
- One `get_stats()` call is enough. Do not export full conversations — that is `stage-conversion-analysis`.
...
- The skill assumes the user's stage list matches the 7-stage LinkNinja default. If `list_stages()` returns custom stages, use those names and order — the per-stage row pattern still applies.
- If `get_stats()` returns zero conversations, render "Pipeline empty — nothing to snapshot yet." and skip the baseline write (don't pollute the file with zeros).
- Caps in the routine recipe are deliberate — six tool calls covers `get_context`, `get_stats`, baseline read/write, and one optional `search_conversations` for the heads-up. Bigger caps are an antipattern for a snapshot skill.
